...
SPREADSHEET_ID = "1sCVKMH_n-Uc-LLPxKhUi1g-XmKVOSlUQZGwrsrDVP_4"
RANGE_NAME = "D1"
UNIQUE_CHECK_RANGE = "C2:C"  # Range to check for uniqueness in the first column
...
def add_to_database():
  ...
  # time.
  if os.path.exists("scripts/webapp/database/token.json"):
  ...
  try:
    ...
    # Get existing values and create a set for fast lookup.
    # result = sheet.values().get(spreadsheetId=SPREADSHEET_ID, range=UNIQUE_CHECK_RANGE).execute()
    # existing_values = result.get("values", [])
    # existing_set = {item[0] for item in existing_values if item}
    
    # # Prepare data to append.
    # ChemBL_ID = results['info']['ChEMBL ID']
    # name = results['name']
    # smiles = results['smiles']
    # descr = Descriptors.CalcMolDescriptors(results['mol'])
    ...
    # models_agreement = results['pred_result']['agreement']

    # Get all values from the 'descr' dictionary as a list.
    # all_descr_values = list(descr.values())
    ...
    # # Filter for unique values to append.
    # unique_values_to_append = []
    # for row in values_to_add:
    #     if row[2] not in existing_set:
    #         unique_values_to_append.append(row)
    #     else:
    #         print(f"Skipping duplicate entry: {row[1]}")
    
    # if not unique_values_to_append:
    
    #     print("No unique data to append.")
    #     return
