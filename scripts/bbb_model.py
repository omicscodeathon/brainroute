...
import datetime
from spmm.dataset import SMILESDataset_BBBP, SMILESDataset_BACEC, SMILESDataset_DILI
from torch.utils.data import DataLoader
import torch.optim as optim
from spmm.scheduler import create_scheduler
import torch.nn.functional as F
import random
from sklearn.metrics import roc_auc_score, recall_score, accuracy_score
import torch.nn as nn
from spmm.xbert import BertConfig, BertForMaskedLM
...
class SPMM_classifier(nn.Module):
    ...
    def forward(self, text_input_ids, text_attention_mask, value, eval=False):
        vl_embeddings = self.text_encoder.bert(text_input_ids, attention_mask=text_attention_mask, 
    ...
    def predict(self, smiles, device='cuda'):
        """
        ...
        Returns:
            dict: {'prediction': int, 'confidence': float, 'probabilities': list, 'bbb_permeable': bool}
        """
        self.eval()
        ...
        with torch.no_grad():
            # Tokenize input
            text_input = self.tokenizer([smiles], padding='longest', truncation=True, 
                                       max_length=100, return_tensors="pt").to(device)
            
            # Get logits
            logits = self.forward(text_input.input_ids[:, 1:], text_input.attention_mask[:, 1:], 
                                 None, eval=True)
            ...
            return {
                ...
                'bbb_permeable': bool(prediction == 1)
            }
    
    def predict_batch(self, smiles_list, device='cuda', batch_size=32):
        """
        ...
        with torch.no_grad():
            for i in range(0, len(smiles_list), batch_size):
                ...
                # Tokenize batch
                text_input = self.tokenizer(batch_smiles, padding='longest', truncation=True,
                                           max_length=100, return_tensors="pt").to(device)
    ...
    def confidence_threshold(self):
        """Default confidence threshold for reliable predictions"""
        return 0.7
...
def evaluate(model, data_loader, tokenizer, device):
    ...
    if isinstance(data_loader.dataset, SMILESDataset_DILI):
...
def main(args, config):
    ...
    if name == 'bace':
        ...
        dataset_train = SMILESDataset_BBBP('./train.csv')
        dataset_val = SMILESDataset_BBBP('./validation.csv')
        dataset_test = SMILESDataset_BBBP('./test.csv')
    elif name == 'lidi':
    ...
    if best_model_state is not None:
        save_dict = {
            ...
            'tokenizer_vocab': args.vocab_filename,
            'config': config,
            'best_valid_auroc': best_valid,
            'best_test_auroc': best_test
        }
        torch.save(save_dict, 'spmm_classifier.pth')
        print('Model saved as spmm_classifier.pth')
        
        # Test prediction functionality
        model.load_state_dict(best_model_state)
        model.eval()
        test_smiles = "c1ccccc1"  # Benzene
        result = model.predict(test_smiles, device=args.device)
        print(f"\nTest prediction for benzene (c1ccccc1):")
        print(f"  BBB Permeable: {result['bbb_permeable']}")
        print(f"  Confidence: {result['confidence']:.4f}")
        print(f"  Probabilities: [Non-permeable: {result['probabilities'][0]:.4f}, Permeable: {result['probabilities'][1]:.4f}]")
